...
import { DrawerComponent } from './component/index';
...
export class SharedService {
  ...
  showMsg(parentEl: ViewContainerRef, msg: string, direction: string = 'top', cls: string = 'red') {
    let config = {message: msg, direction: direction, cls: cls, zIndex: 1001};
    let cmp: any = this.showDrawer(parentEl, config);
  ...
  showDrawer(parentEl: ViewContainerRef, config: any, show: boolean = true) {
    let cmp: any = this.addComponent(DrawerComponent, config, parentEl);
